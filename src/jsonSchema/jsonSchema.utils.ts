...
import type { 
  JsonSchemaFragment, JsonSchemaNodeValue, JsonSchemaNodeKind, JsonSchemaNodeMeta, JsonSchemaNodeType, 
  JsonSchemaTransformedFragment, JsonSchemaTreeNode 
} from './jsonSchema.types'
import { jsonSchemaCommonProps, jsonSchemaNodeValueProps, jsonSchemaNodeTypes } from './jsonSchema.consts'
...
export function inferTypes(fragment: JsonSchemaFragment): JsonSchemaNodeType[] {
  ...
  for (const type of Object.keys(jsonSchemaNodeValueProps) as JsonSchemaNodeType[]) {
    const props = jsonSchemaNodeValueProps[type].slice(jsonSchemaCommonProps.length)
...
export const isJsonSchemaTreeNode = (node?: ModelDataNode<JsonSchemaNodeValue<any>, JsonSchemaNodeKind, JsonSchemaNodeMeta>): node is JsonSchemaTreeNode<any> => {
  return !!node && node.type === modelTreeNodeType.simple
}

export const isJsonSchemaComplexNode = (node?: ModelDataNode<JsonSchemaNodeValue<any>, JsonSchemaNodeKind, JsonSchemaNodeMeta>): node is JsonSchemaTreeNode<any> => {
  return !!node && node.type !== modelTreeNodeType.simple
}
